...
if(isset($_POST['add']))
	{
		...
		$compulsory_subject=$db->escape($_POST['compulsory_subject']);
		@$exploide=explode("and",$class_section);
		if(!empty($id) && !empty($classname) && $class_section!="Select One" )
		{
			$query="INSERT INTO `exam_type_info`(`exam_id`,`select_class`,`exam_type`,`compulsory_subject`) VALUES ('".$fetch[0]."','$exploide[0]','$classname','$compulsory_subject')";
...
if(isset($_POST['Update']))
	...
	$compulsory_subject=$db->escape($_POST['compulsory_subject']);
		...
		{
			$query="REPLACE INTO `exam_type_info` (`exam_id`,`select_class`,`exam_type`,`compulsory_subject`) VALUES ('$id','$exploide[0]','$classname','$compulsory_subject')";
		...
		while($fetch_class=$result_class->fetch_array()){
					$table.="<tr>"."<td colspan='3' align='center'>"."<span class='text-Warning' style='font-size:15px; font-weight:blod;'>".$fetch_class[4]."</span>"."</td>"."</tr>";	
						...
						while($fetch_all=$result_all->fetch_array()){
								$table.="<tr>"."<td  class='text-center text-danger'>".$fetch_all[2]."</td>"."<td  class='text-center text-danger'>".$fetch_all[3]."</td>"."<td class='text-center'>";
								
								$table.="<a href='?edit=$fetch_all[0]&cs=$fetch_all[1]'class='btn btn-primary'onclick='return confirm_click()' style='width:80px'>Edit</a><a href='?delete=$fetch_all[0]&cs=$fetch_all[1]'class='btn btn-danger'onclick='return confirm_delete()' style='width:80px'>Delete</a>";
									$table.="</td>"."</tr>";
								}
...
?>
  	...
  	<table align="center" class="table table-responsive box" style="border:1px #CCCCCC solid; margin-top:30px; color: #000;">
	    	...
	    	<tr>
				<td class="info">Compulsory Subject  </td>
				<td class="info">:</td>
				<td class="info">
					<div class="col-lg-12 has-warning">
						<input type="number" placeholder="Compulsory Subject" value="<?php echo $fetch[3];?>" class="form-control" name="compulsory_subject"  />
						<span class="glyphicon glyphicon-warning-sign form-control-feedback"></span>
					</div>
				</td>
				<td class='info'><span class="text-danger text-justify"><strong>*&nbsp;Try It English</strong></span></td>
