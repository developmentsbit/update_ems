	...
	<table width="510" align="center"  class="table table-bordered  table-responsive table-hover" style="margin-top:30px;">
		<tr align="center">
		  ...
		  <td width="147">ID</td>
				...
				<td width="121">Ex-Roll</td>
				<td width="121">New Roll </td>
		...
		@$Gender=$_REQUEST['Gender'];
				   ...
				   $sqlForAll="SELECT `student_personal_info`.`student_name`,`religious`,`student_acadamic_information`.`session2`,`student_acadamic_information`.`id`,`running_student_info`.`class_roll`  FROM `student_acadamic_information`
...
WHERE `student_acadamic_information`.`session2`='".$_POST["session"]."' AND `student_acadamic_information`.`admission_disir_class`='$explode[0]' AND `student_acadamic_information`.`admission_disir_group`='$explodegp[0]' AND `student_personal_info`.`gender`='$Gender' AND `running_student_info`.`class_roll` BETWEEN ".$_POST["from"]." and ".$_POST["to"]."
 ORDER BY `running_student_info`.`class_roll` ASC ";
			...
			if($resultForAll){
				...
				$fetchRoll = "SELECT `meritPosition` FROM `result` WHERE  `examId`='$examtype[0]' AND `session`='".$_POST["session"]."' AND  `STD_ID`='".$fetchForAl['id']."'";
		...
		?>
		<tr>
				<td align="center"><input type="checkbox" class="check_elmnt" name="chek[]" id="chek-<?php echo $fetchForAl["id"];?>" value="<?php echo $fetchForAl["id"];?>" onclick="return byroollcheked('<?php echo $fetchForAl["id"];?>')" /></td>

				<td><?php echo $fetchForAl["id"];?></td>

				<td style=" text-transform: uppercase;"><?php echo $fetchForAl["student_name"];?></td>

				<td align="center"><?php echo $fetchForAl['class_roll']?></td>
				<td align="center"><input type="text" style="width:50px;" class="roll" name="rollNo[]" id="rollNo-<?php echo $fetchForAl["id"];?>" <?php if($resultroll){
					 ?> value="<?php echo $fetchRoll[0];?>" <?php } ?> required  disabled="disabled"/> </td>
				<td align="center"><?php echo $fetchForAl["religious"];?></td>
				<td align="center"><input type="text" id="year[]" name="year[]" value="2023" style="padding-left:15px; width:120px;" /></td>
		</tr>
		<?php } } ?>
			<tr>
