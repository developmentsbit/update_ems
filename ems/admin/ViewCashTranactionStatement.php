...
<html lang="en">
  <head>
    ...
    <title>Case Transaction Statement</title>
    ...
    <p style="margin-top:-13px;font-size:14px;font-family:Arial, Helvetica, sans-serif"><?php print $fetchinfo['phone_number']?> ,<?php print $fetchinfo['email']?>  </p>
     ...
     <tr>
          <td colspan="3" align="center">
             ...
             <h3>Case Transaction Statement</h3>
...
$sql=$db->link->query("SELECT * FROM `cash_receiver_info` WHERE `id`='".$_GET['sideNoteTitle']."'");
...
$select=$db->link->query("SELECT `date`,`receiversName`,`details`,`payable_amount`,`Cash_payment` FROM `cash_received_withdraw` WHERE `supplier_id`='".$_GET['sideNoteTitle']."'  ORDER BY `id` ASC ");
