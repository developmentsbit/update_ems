...
AND `result`.`session`='$session' GROUP BY `result`.`session` order by `result`.`std_roll` ASC";
		...
		{
		    $stdid=$_GET["stdid"];
		    $db->link->query("DELETE FROM `marksheet` WHERE `STudentID`='$stdid'  AND `ClassId`='$clId' AND `GroupID`='$gpId' AND `ExamId`='$examId'");
		    $db->link->query("DELETE FROM `gnerate_marks` WHERE `studentID`='$stdid' AND `ClassID`='$clId' AND `GroupID`='$gpId' AND `ExamID`='$examId' AND `session`='$session'");
		    $db->link->query("DELETE FROM `result` WHERE `STD_ID`='$stdid' AND `classId`='$clId' AND `GroupID`='$gpId' AND `examId`='$examId' AND `session`='$session'");
		   // echo  "DELETE FROM `result` WHERE `STD_ID`='$stdid' AND `classId`='$clId' AND `GroupID`='$gpId' AND `examId`='$examId' AND `session`='$session'";
...
<html lang="en">
  <head>
	...
	{
	    var con=confirm("do you want to confirm delete?");
		if(con==true)
		{
		    return true;
		}
...
<td  style=" border-bottom:1px #000 solid;" valign="top" align="center">
	<table style="padding: 0px; margin: 0px; ">
			<tr>
				<td align="center"> 
				<span>
					<img src="all_image/logoSDMS2015.png" style=" width: 80px; height: 80px">
				</span>
			</td>
			<td align="center">
						...
						<table width="100%" height="100%"  cellpadding="0" cellspacing="0"  style=" padding: 0px; margin: 0px; width: 120px;">
								...
								<tr>
									<td  align="center" valign="middle">
									    ...
									    <a onclick="return confirmdelete()" class="noneBtnForprin" href="ViewTabulationSheet2.php?clID=<?php print $clId?>&exId=<?php print $examId?>&session=<?php print $session?>&gpna=<?php print $gpId?>&stdid=<?php print $fetchSelectName['STudentID']?>">Delete</a>
...
?>
	...
	<td style="border-right:1px #000 solid; border-bottom:1px #000 solid; width: 130px; " valign="top">
				...
				<table  cellpadding="0" cellspacing="0"  style="height: 100%; border-top: 0px; width: 150px;"  height="100%" >
								...
								<tr>
										<td style="border-bottom:1px #000 solid; height: 36px; text-align: center; font-size:16px; ">
								<?php
										if($subName==1)
											{
												print "<span style=''>".$fetchSubjectPart['subject_part_name']."&nbsp;(". $fetchSubjectPart['subject_part_code'].")"."</span>";
											}
											else
											{
				 ...
				 <td align="center" style="border-bottom: 1px #000 solid;" colspan="4" ><?php echo $fetchSubjectPartmarks[4]; 
// obtainMarks
...
?>
				...
				<td style="border-right:1px #000 solid; border-bottom:1px #000 solid; width: 150px;  " valign="top">
								...
								<tr>
										<td style="border-bottom:1px #000 solid; height: 36px; text-align: center;  background: #fff; font-size:16px;"><?php

										if($subName==1)
											{
												 print "<span>".$fetchSubjectPartmarks['subject_name']."&nbsp;(". $fetchSubjectPartmarks['subject_code'].")"."</span>";
											}
											else
											{
								...
								?>>
										...
										<table width="100%"  cellpadding="0" cellspacing="0"  style="height: 100%">
										  	...
										  	<tr>
										  	  ...
										  	  <td align="center"  colspan="4" style="border-bottom: 1px #000 solid;"><?php echo $fetchSubjectPartmarks[4]; 
...
<?php if($fetchSubjectPartmarks[5]=='F') {?>
								...
								> <b> <?php echo $fetchSubjectPartmarks[5]; ?></b></td>
  </tr>
										  	<tr>
										  	  <td align="center"  style="font-weight: bold;"><?php echo $fetchSubjectPartmarks[6]; ?></td>
  </tr>
