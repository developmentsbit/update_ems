					...
					if(!empty($_POST["fee"])){
							...
							for($a=0;$a < count($_POST["fee"]);$a++){
								 ...
								 $feeid=explode("and",$_POST['fee'][$a]);
								 	...
								 	$insertSql="INSERT INTO `student_account_info` (`studentID`,`class_id`,`fee_id`,`year`,`admin_id`) VALUES('$stdId','$className[0]','$feeid[1]','$year','".$_SESSION["id"]."')";
...
<?php
		...
		?>
				<table width="100%" align="center"  class="table table-bordered  table-responsive table-hover" style="margin-top:30px;">
		<tr align="center">
				<td width="50">Select All<BR/>
					<input id="chkbx_all"  onclick="return check_all()" type="checkbox"  checked="checked" />
		  </td>
				<td width="100">Name</td>
				<td width="20">Roll No</td>
				<td width="500">Select Fee</td>
...
WHERE  `running_student_info`.`class_id`='$expolde[0]' AND `student_acadamic_information`.`session2`='".$_POST["session"]."'  ORDER BY `running_student_info`.`class_roll` ASC   LIMIT ".$_POST["from"].",".$_POST["to"]."";
		...
		?>
		<tr>
				<td><input type="checkbox" class="check_elmnt"  checked="checked"  name="chek[]" id="chek-<?php echo $fetchForAl["student_id"];?>" value="<?php echo $fetchForAl["student_id"];?>" onclick="return byroollcheked('<?php echo $fetchForAl["student_id"];?>')" /></td>
				...
				<?php
						...
						$resultFees=$db->select_query($selectFees);
							if($resultFees->num_rows > 0){
									...
									?>
									<input type="checkbox" id="disableId-<?php echo $fetchForAl["student_id"];?>" checked="checked"  name="checkbox[]" class="feechck" value="<?php echo $fetchFees['id'].'and'.$fetchForAl["student_id"];?>"   checked /> &nbsp;&nbsp;<?php echo $fetchFees['title'];?>&nbsp;&nbsp;
									<?php
								}	}
							}
				?>
