...
// echo $datachek;
						...
						<table width="77%" class="table table-bordered" style="width:100%;">
								<tr align="center">
										...
										<td width="12%"><strong>Prev. Discount</strong></td>
										<td width="12%"><strong>Paid</strong></td>
										<td width="11%"><strong>Discount</strong></td>
										<td width="17%"><strong>Payable </strong></td>
	   ...
	   $alreadyAddDiscount="SELECT SUM(`discount`) FROM `add_discount` WHERE `student_id`='$fetchchek[0]'  AND `class_id`='$fetchchek[1]' AND `year`='".$_POST["year"]."' AND `feeid`='$fetchchek[2]'";
	   $checkAllreadyDiscountAdd=$db->select_query($alreadyAddDiscount);
							if($checkAllreadyDiscountAdd)
							{
							    $fetch_dis=$checkAllreadyDiscountAdd->fetch_array();
							}
							
							
	   $totalPaid="SELECT SUM(`paid_amount`) FROM `student_paid_table` WHERE `student_id`='$fetchchek[0]'  AND `class_id`='$fetchchek[1]' AND `year`='".$_POST["year"]."' AND `fk_fee_id`='$fetchchek[2]'
";
	   $paidFee=$db->select_query($totalPaid);
							if($paidFee)
							{
							    $fetch_paid=$paidFee->fetch_array();
							}
							
							$totalpaidAmount=$fetch_dis[0]+$fetch_paid[0];
							
							if($fetchchek["Common_Exceptional"]=="Common")
										 {
										 	$feeAmount=$fetchchek["amount"];

										 }
										 else
										 {
										    $feeAmount=$fetchchek["AmountExceptional"];
										 }
										 $Total_Fee_Amount=$Total_Fee_Amount+$feeAmount;
							$Total_paid_Amount+=$fetch_paid[0];
								$Total_discount_Amount+=$fetch_dis[0];
										...
										<td>
										 ...
										 <td>
										 	<input type="text" name="ammount[]" id="ammount-<?php echo $fetchchek["fee_id"]; ?>" value="<?php 
										 		echo $feeAmount-$totalpaidAmount;
										  ...
										  <td>
										 	<input type="text" name="preDiscount[]" id="prevDiscount-<?php echo $fetchchek["fee_id"]; ?>"   style="width:100%; height:30px; text-align:center" disabled="disabled" value="<?php print $fetch_dis[0]?>" />
										 </td>
										 
										 <td>
										 	<input type="text" name="prevPaid[]" id="prevPaid-<?php echo $fetchchek["fee_id"]; ?>" style="width:100%; height:30px; text-align:center" disabled="disabled"   value="<?php print $fetch_paid[0]?>"/>
										 </td>
						...
						<?php
						 	...
						 	<tr>
								    ...
								    <td colspan="2"  align="right">Total Amount : <?php print $Total_Fee_Amount;?></td>
								 
								    <td colspan="2" align="right">Total Paid: <?php echo $Total_paid_Amount;?></td>
								 
								    <td colspan="2"  align="right">Total Discount:<?php echo 	$Total_discount_Amount;?></td>
								  
								    <td colspan="2" align="right">Due :<?php echo 	$Total_Fee_Amount-($Total_paid_Amount+$Total_discount_Amount);?></td>
								</tr>
</table>
