...
if(isset($_POST['add']))
{
		...
		$receiversName = $db->escape($_POST['receiversName']);
		$id=$db->withoutPrefix('cash_received_withdraw','id','C','8');

		if(!empty($sideNoteTitle))
		{
			$query="INSERT INTO `cash_received_withdraw`(`id`,`date`,`supplier_id`,`payable_amount`,`Cash_payment`,`due_amount`,`details`,`entry_date`,`type`,`receiversName`)
VALUES('$id','$Date','$sideNoteTitle','','$Amount','','$Details','".date('Y-m-d')."','paid','$receiversName')";
...
if(isset($_POST['Update']))
	{
		...
		if(!empty($sideNoteTitle))
		{
			$query="REPLACE INTO `cash_received_withdraw`(`id`,`date`,`supplier_id`,`payable_amount`,`Cash_payment`,`due_amount`,`details`,`entry_date`,`type`,`receiversName`)
VALUES('$id','$Date','$sideNoteTitle','','$Amount','','$Details','".date('Y-m-d')."','paid','$receiversName')";
			$result=$db->link->query($query);
			if($result)
			{
			...
			print "<script>location='cashWithdraw.php'</script";
			}
		}
		else
		{
...
if(isset($_POST['View']))
	{
		$query="SELECT `cash_received_withdraw`.`id`, `cash_received_withdraw`.`date`,`cash_receiver_info`.`name`,`cash_received_withdraw`.`Cash_payment`,`cash_received_withdraw`.`details`,`cash_received_withdraw`.`receiversName` FROM `cash_received_withdraw`
INNER JOIN `cash_receiver_info` ON `cash_receiver_info`.`id`=`cash_received_withdraw`.`supplier_id` where `type`='paid' ORDER BY `cash_received_withdraw`.`id` ASC  ";
		$result=$db->select_query($query);

		$table.="<br><table class='table table-bordered'>"."<tr class='success'>"."<td >Date </td>"."<td >Name</td>"."<td >Details</td>" ."<td >Withdraw Amount </td>"."<td>Receiver's Name</td>"."<td>Action</td>"."</tr>";
		if($result)
		{
		...
		while($a=$result->fetch_array())
		{
			   $table.="<tr>";
				...
				$table.="<td>".$a['Cash_payment']."</td>";
				$table.="<td>".$a['receiversName']."</td>";
...
<html lang="en">
  ...
  <div class="panel-heading" style="font-size: 16px; color: #16a085; text-transform: uppercase;"><b>Cash Withdraw</b></div>
  <div class="panel-body">
   ...
   <div class="form-group">
	...
	<input type="text"  placeholder="Amount" style="height: 40px; border-radius: 0px;" autocomplete="off" class="form-control" name="Amount" value="<?php echo $fetch[4];?>"  />
  </div>     
	...
	<input type="text"  placeholder="Receiver's Name" style="height: 40px; border-radius: 0px;" autocomplete="off" class="form-control" name="receiversName" value="<?php echo $fetch[9];?>"  />
  </div>    
