<?php
	...
	$fetch[5]="";
	$prefix=date("y"."m"."d");
	$fetch[0]=$db->withoutPrefix('colume_wise_expense','id',$prefix,'10');
...
if(isset($_POST['add']))
	{
		...
		if(!empty($_POST['checkbox']) && !empty($title) )
		{
			 ...
			 for($x = 0 ;$x < $countFees ; $x++){
			  ...
			  $query="INSERT INTO `colume_wise_expense` VALUES ('".$fetch[0]."','$title','".$_POST['checkbox'][$x]."')";
			 // print $query;
...
//view data...............................	
	if(isset($_POST['View']))
	{
        ...
        $table.="<tr class='warning' >"."<td align='left' colspan='4'>"."<a href='coloumWiseExpenseTitle.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."<span class='text-success' style='font-size:18px; padding-left:380px; font-weight:blod;'>Column Wise Expense Title</span>"."</td>"."</tr>";
 ...
 $selectFeeName = "SELECT `colume_wise_expense`.`id`,`income_expense_title`.`title` FROM `colume_wise_expense` 
INNER JOIN `income_expense_title` ON `income_expense_title`.`id`=`colume_wise_expense`.`fk_fee_id`
WHERE `colume_wise_expense`.`fk_column_id`='$fetchColoumName[0]'";
...
//end link edit data..........................
	...
	if(isset($_GET['dlt']))
	{
		...
		$query="DELETE FROM `colume_wise_expense` WHERE `id`='$linid'";
		$delete=$db->delete_query($query);
		$fetch[0]=$db->withoutPrefix('colume_wise_expense','id',$prefix,'10');
		print "<script>location='coloumWiseExpenseTitle.php'</script>";
...
?>
  	...
  	<table align="center" class="table table-responsive box" cellpadding="0" cellspacing="0" style="border:1px #CCCCCC solid; margin-top:30px; color: #000;">
  			<tr>
				...
				<span style="font-size:22px; color:#333; display:block;">Column Wise Expense Title Setup </span>  <br>	
				</td>			
			</tr>
			...
			<tr>
				...
				<td width="81%" class="info">
					<div class="col-lg-6 has-warning">
								...
								<?php 
								$selMonth = "SELECT * FROM `expense_coloumn_setup` ORDER BY `sl` ASC";
			...
			<tr>
				<td class="info" colspan="5" >
					...
					$expenseTitle = " SELECT * FROM `income_expense_title` WHERE `type`='Expense' AND `id` NOT IN (SELECT `fk_fee_id` FROM `colume_wise_expense`) ORDER BY `index` ASC  ";
