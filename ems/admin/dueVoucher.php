	...
	$db = new database();
 		...
 		if(isset($_POST["ResultGenerate"])){
			...
			if(isset($classID) and $classID != "Select One" and isset($examtype) and $examtype != "Select Exam Name" and isset($Session) and $Session != "Select Session" ){
					print "<script>location='dueBlankReport.php?clID=$classID[0]&session=$Session&gpna=$groupname[0]&stdRollfrom=$stdRollfrom&stdRollto=$stdRollto&exam=$examtype[0]'</script>";
			}else{
  ...
  <body>
  	<form name="teacherinfo" action="" method="post"  enctype="multipart/form-data" class="form-horizontal marksEditEntry" >
            <div class="col-md-8 col-md-offset-2" id="AddMarksStep1"><br/> <br/>
                <span class="glyphicon glyphicon-arrow-right text-justify text-uppercase text-warning" style="font-weight: bold; padding-top: 15px; font-size: 18px;"><strong style="padding-left:5px">Due Blank Report </strong></span><br/>
...
<input type="text" name="to" id="to"  style="width:120px; background:#F7071D; border:none; font-weight:bold; color:#fff; height:25px;  text-align:center;"/>
                ...
                <tr><td colspan="2" align="center"><input type="submit" name="ResultGenerate" value="Show" class="btn btn-success btn-md" style="width: 180px" formtarget="_blank"></input>
