        ...
        <tr>
            <td align="center"style="border-right:1px #000 solid; " >
                ...
                <table style="width: 100%;">
                    ...
                    <?php
$query="SELECT * FROM `expense_coloumn_setup`  ORDER BY `sl` ASC";
   
   
     if(isset($_GET["date"]))
    {
        $query="SELECT * FROM `income_expense_title`
INNER join `other_cost` on `other_cost`.`title`=`income_expense_title`.`id`
WHERE `type`='Expense' and `other_cost`.`date`='$date' GROUP BY `other_cost`.`title` ORDER BY `index` ASC";
    ...
    if(isset($_GET["monthID"]))
    {
      ...
      $mo=$_GET["year"].'-'.$_GET["monthID"];
          ...
          $query="SELECT * FROM `income_expense_title`
INNER join `other_cost` on `other_cost`.`title`=`income_expense_title`.`id`
WHERE `type`='Expense' and `other_cost`.`date` LIKE '%$mo%' GROUP BY `income_expense_title`.`id` ORDER BY `index` ASC";
            ...
            $count=mysqli_num_fields($SelectClass);
                 ...
                 <tr>
                   <td style=" border-left: 1px #000 solid; padding: 10px;border-bottom: 1px #000 solid;">
                        ...
                        <td style=" border-left: 1px #000 solid; padding: 10px;border-bottom: 1px #000 solid;">
                         <a href="ExpenseVoucher.php?id=<?php print $fetch_Class[0]; ?>"  target="_blank">   <?php print $fetch_Class[0]; ?></a>
...
WHERE `type`='Expense' and `other_cost`.`date` LIKE '%$mo%' GROUP BY `income_expense_title`.`id` ORDER BY `index` ASC";
 ...
 if(isset($_GET["monthID"]))
      ...
      if(isset($_GET["SelectYear"]))
    {
        ...
        $y=$_GET["SelectYear"];
        $totalAmunt="SELECT * FROM `other_cost` 
INNER JOIN `colume_wise_expense` ON `colume_wise_expense`.`fk_fee_id`=`other_cost`.`title`
WHERE  `other_cost`.`id`='".$fetch_Class[0]."' AND `colume_wise_expense`.`fk_column_id`='$fetch_all[0]' AND `other_cost`.`date` LIKE '%$y%' ";
...
WHERE `type`='Expense' and `other_cost`.`date` LIKE '%$mo%' GROUP BY `income_expense_title`.`id` ORDER BY `index` ASC";
 ...
 $totalAmunt="SELECT SUM(`amount`) FROM `other_cost` WHERE `title`='".$fetch_all[0]."' AND `date` LIKE '%$mo%'";
      ...
      if(isset($_GET["SelectYear"]))
    {
      ...
      $y=$_GET["SelectYear"];
      $mo=$_GET["year"].'-'.$_GET["monthID"];
 $totalAmunt="SELECT SUM(`amount`)  FROM `colume_wise_expense` 
INNER JOIN `other_cost` ON `other_cost`.`title`= `colume_wise_expense`.`fk_fee_id`
 WHERE `colume_wise_expense`.`fk_column_id`='".$fetch_all[0]."' AND `date` LIKE '%$y%'";
