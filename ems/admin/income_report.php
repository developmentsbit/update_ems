...
<!-- ///////////////////////Print Title/////////////// -->
     	<tr>
     		...
     		<td style=" border-left: 1px #000 solid;border-bottom: 1px #000 solid; text-align: center; width: 150px;"> 
...
$i=0;
			$count=mysqli_num_fields($result_all);
						while($fetch_all=$result->fetch_array()){
     					...
     					<td style="border-bottom: 1px #000 solid; border-left: 1px #000 solid; padding: 10px;
                         text-align: center;">
...
<tr>
     ...
     if(isset($_GET["monthID"]))
 {  
     $class="SELECT * FROM `student_paid_table` WHERE `month`='".$_GET["monthID"]."' AND `year`='".$_GET["year"]."'  GROUP BY `date` ORDER BY `date` ASC ";
...
if(isset($_GET["SelectYear"]))
 {  
     $class="SELECT * FROM `student_paid_table` WHERE `year`='".$_GET["SelectYear"]."'  GROUP BY `date` ORDER BY `date` ASC ";
...
<?php
 ...
 if(isset($_GET["monthID"]))
 {
 	...
 	$totalReceivedAmount="SELECT SUM(`paid_amount`) AS 'Totalotal'  FROM `student_paid_table`
INNER JOIN `columnwisefeesetupforstd` ON `columnwisefeesetupforstd`.`fk_fee_id` = `student_paid_table`.`fk_fee_id`
 WHERE `columnwisefeesetupforstd`.`fk_column_id`='".$fetch_all[0]."' AND `student_paid_table`.`month`='".$_GET["monthID"]."' AND `student_paid_table`.`year`='".$_GET["year"]."' AND `student_paid_table`.`date`='".$fetch_Class[4]."'";
 ...
 if(isset($_GET["SelectYear"]))
 {
	   ...
	   $totalReceivedAmount="SELECT SUM(`paid_amount`) AS 'Totalotal'  FROM `student_paid_table`
INNER JOIN `columnwisefeesetupforstd` ON `columnwisefeesetupforstd`.`fk_fee_id` = `student_paid_table`.`fk_fee_id`
 WHERE `columnwisefeesetupforstd`.`fk_column_id`='".$fetch_all[0]."'  AND `student_paid_table`.`year`='".$_GET["SelectYear"]."' AND `student_paid_table`.`date`='".$fetch_Class[4]."'";
