...
.style8 {color: #000066;font-size:14px;font-family:Arial, Helvetica, sans-serif}
     		...
     		<td align="center"style="border-right:1px #000 solid; " >
     			...
     			<table style="width: 100%;">
     				<tr>
     					...
     					<td style="width:300px; border-left: 1px #000 solid;border-bottom: 1px #000 solid; text-align: center; width:400px;" width="400" > মাস </td>
  ...
  <?php
    ...
    <td style="border-bottom: 1px #000 solid; border-left: 1px #000 solid; padding: 10px; text-align: center;">
     ...
     <?php
        $query="select * from `expense_coloumn_setup` order by `id` ASC ";
        ...
        ?>
             ...
             <td style="border-bottom: 1px #000 solid; border-left: 1px #000 solid; padding: 10px; text-align: right;">
                  ...
                  $date=$_GET['year'].'-'.$fetch_Month[0];
                        $columeFeesColle=$db->link->query("SELECT `colume_wise_expense`.`fk_fee_id`, SUM(`other_cost`.`amount`) FROM `colume_wise_expense`
INNER JOIN `other_cost` ON `other_cost`.`title`=`colume_wise_expense`.`fk_fee_id`
WHERE `colume_wise_expense`.`fk_column_id`='$fetch_all[0]' AND `other_cost`.`date` LIKE '%$date%'");
...
$columeFeesColle=$db->link->query("SELECT `colume_wise_expense`.`fk_fee_id`, SUM(`other_cost`.`amount`) FROM `colume_wise_expense`
INNER JOIN `other_cost` ON `other_cost`.`title`=`colume_wise_expense`.`fk_fee_id`
WHERE `colume_wise_expense`.`fk_column_id`='$fetch_all[0]' AND `other_cost`.`date` LIKE '%$year%'");
if($columeFeesColle)
{
        $fetch_fees=$columeFeesColle->fetch_array();
...
$monthlyCollection=$db->link->query("SELECT `colume_wise_expense`.`fk_fee_id`, SUM(`other_cost`.`amount`) 
FROM `colume_wise_expense` 
INNER JOIN `other_cost` ON `other_cost`.`title`=`colume_wise_expense`.`fk_fee_id` 
WHERE  `other_cost`.`date` LIKE '%$date%'");
...
<?php
        ...
        <?php 
        $year=$_GET["year"];
            $yearlyCollection=$db->link->query("SELECT SUM(`amount`) FROM `other_cost` WHERE `Entry_Date` LIKE '%$year%'");
