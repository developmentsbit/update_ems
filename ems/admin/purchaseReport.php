...
$totalpaid=0;
  ...
  $sql=$db->link->query("SELECT SUM(`payable_amount`) AS 'payable', SUM(`Cash_payment`) AS 'paid',`name`,`address`,`phone` FROM `supplier_payment` 
INNER JOIN `supplier_info` ON `supplier_info`.`id`=`supplier_payment`.`supplier_id`
 GROUP BY `supplier_id` ORDER BY `supplier_info`.`sl` ASC ");
