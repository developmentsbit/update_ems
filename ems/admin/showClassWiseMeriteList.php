            ...
            <div class="col-md-10 col-md-offset-1"  style="border:2px #CCCCCC solid; margin-top:10px;"><br/> <br/>
                ...
                <table cellpadding="0" cellspacing="0"  class="table table-responsive table-hover " style="margin-top:0px;">					
							...
							<tr class="" style="border:1px #CCCCCC solid">
									<td colspan="11" align="center"><span class="text-justify text-danger" style="font-size:16px;"><strong>  Merit List </strong></span> </td>
							</tr>
...
//echo $select;
						...
						if($resultquery)
						{
							...
							$sql1= "SELECT `totalMarks` FROM `result` WHERE `result`.`classId`='$clId' AND `result`.`session`='$session' AND `result`.`examId`='$examId' AND `result`.`CGPA`=$fetch_gpa[0] GROUP BY `totalMarks` ORDER BY `totalMarks` DESC";
//echo $sql1;
...
INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`result`.`STD_ID`  WHERE `result`.`classId`='$clId'  AND `result`.`session`='$session'  AND `result`.`examId`='$examId' AND `result`.`CGPA`=$fetch_gpa[0] AND `totalMarks`=$fetchTotalMarks[0]  ORDER BY `std_roll` ASC  ";
