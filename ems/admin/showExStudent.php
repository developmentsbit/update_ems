...
AND `class_roll` BETWEEN '".$_POST["from"]."' AND '".$_POST["to"]."' 
 ...
 $check_query=$db->select_query($selectAllData);
  ...
  <tr align="center">
    ...
    <td align='left'><?php echo $fetch_class['student_name']?>  </td>
...
INNER JOIN  `student_personal_info` ON `student_personal_info`.`id`=`running_student_info`.`student_id` WHERE `running_student_info`.`student_id`='".$_POST['fromData'][$a]."'";
						$resultQuery = $db->select_query($select_query);
							if($resultQuery->num_rows > 0 ){
								...
								$update = "UPDATE `student_personal_info` SET `status`='0' WHERE `id`='$fetchQuery[student_id]'";

									$db->insert_query($update);



									 $inserQuery = "INSERT INTO `exstudentreport`(`Id`,`ClassId`,`GroupId`,`classRoll`,`session`,`date`,`year`,`admin`) VALUES('$fetchQuery[student_id]','$fetchQuery[class_id]','$fetchQuery[group_id]','$fetchQuery[class_roll]','$fetchQuery[session2]','".date('Y-m-d')."','".date('Y')."','".$_SESSION["name"]."')";
