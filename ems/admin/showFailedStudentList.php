...
<html lang="en">
  <head>
    ...
    <title>Show Failed Student List</title>
            ...
            <div class="col-md-10 col-md-offset-1"  style="border:2px #CCCCCC solid; margin-top:10px;"><br/> <br/>
                ...
                <table cellpadding="0" cellspacing="0"  class="table table-responsive table-hover " style="margin-top:0px;">					
							...
							<tr class="" style="border:1px #CCCCCC solid">
									<td colspan="7" align="center"><span class="text-justify text-danger" style="font-size:16px;"><strong> Failed Student List </strong></span> </td>
							</tr>
							...
							<tr align="center">
								...
								<td style="border:1px #999999 solid;">Total Marks</td>
...
INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`result`.`STD_ID`  WHERE `result`.`classId`='$clId' AND `result`.`GroupID`='$gpId' AND `result`.`session`='$session' AND `result`.`examId`='$examId' AND `CGPA` = '0.00' ORDER BY `std_roll` ASC ";
							$result1=$db->select_query($sql1);
							...
							<td style="border:1px #999999 solid;"><?php echo $fetchAll['totalFailSub']; ?></td>	
