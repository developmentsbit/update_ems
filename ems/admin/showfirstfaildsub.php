...
INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`result`.`STD_ID` INNER JOIN `student_acadamic_information` ON `student_acadamic_information`.`id`=`result`.`STD_ID` WHERE `result`.`classId`='$clId' AND `student_acadamic_information`.`session2`='$session' AND `result`.`GroupID`='$gpId' AND `result`.`examId`='$examId'  ORDER BY `result`.`std_roll` ASC $limit";
						...
						$sql1= "
SELECT `result`.*,`gnerate_marks`.`subjectID`,`add_subject_info`.`subject_name`,`subject_code` FROM `result` 
INNER JOIN `gnerate_marks` ON `gnerate_marks`.`studentID` = `result`.`STD_ID` 
INNER JOIN `add_subject_info` ON `add_subject_info`.`id`=`gnerate_marks`.`subjectID` 
WHERE `gnerate_marks`.`ClassID`='$clId' AND `gnerate_marks`.`GroupID`='$gpId' AND `gnerate_marks`.`session`='$session' AND `gnerate_marks`.`ExamID`='$examId' GROUP BY `gnerate_marks`.`subjectID` ORDER BY `add_subject_info`.`serial` ASC";
 ...
 //echo $sql1;
							...
							<tr  class="">
								...
								<?php
								//	$taotastd = "SELECT COUNT(*) as 'Total' FROM `running_student_info` WHERE `class_id`='$clId'";
