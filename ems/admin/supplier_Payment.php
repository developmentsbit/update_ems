...
if(isset($_POST['add']))
{
		...
		{
			$query="INSERT INTO `supplier_payment`(`id`,`date`,`supplier_id`,`payable_amount`,`Cash_payment`,`due_amount`,`details`,`entry_date`,`type`,`receiversName`)
VALUES('$id','$Date','$sideNoteTitle','0','$Amount','0','$Details','".date('Y-m-d')."','paid','$receiversName')";
...
if(isset($_POST['Update']))
	{
		...
		if(!empty($sideNoteTitle))
		{
			$query="REPLACE INTO `supplier_payment`(`id`,`date`,`supplier_id`,`payable_amount`,`Cash_payment`,`due_amount`,`details`,`entry_date`,`type`,`receiversName`)
VALUES('$id','$Date','$sideNoteTitle','0','$Amount','0','$Details','".date('Y-m-d')."','paid','$receiversName')";
			$result=$db->link->query($query);
			if($result)
			{
			...
			print "<script>location='supplier_Payment.php'</script";
			}
		}
		else
		{
...
if(isset($_POST['View']))
	{
		$query="SELECT `supplier_payment`.`id`, `supplier_payment`.`date`,`supplier_info`.`name`,`supplier_payment`.`Cash_payment`,`supplier_payment`.`details`,`supplier_payment`.`receiversName` FROM `supplier_payment`
INNER JOIN `supplier_info` ON `supplier_info`.`id`=`supplier_payment`.`supplier_id` where `type`='paid' ORDER BY `supplier_payment`.`id` ASC  ";
		$result=$db->select_query($query);

		$table.="<br><table class='table table-bordered'>"."<tr class='success'>"."<td >Date </td>"."<td >Name</td>"."<td >Details</td>" ."<td >Paid Amount </td>"."<td>Receiver's Name</td>"."<td>Action</td>"."</tr>";
		if($result)
		{
...
<html lang="en">
  ...
  <div class="panel-heading" style="font-size: 16px; color: #16a085; text-transform: uppercase;"><b>Payment</b></div>
