...
WHERE `running_student_info`.`student_id`='".$_GET["stdid"]."'";
	$resultForAll=$db->select_query($sqlForTest);
		...
		if(isset($_POST['save']))
		{
		    $sql=$db->link->query("INSERT INTO `tc_reg`(`class_name`, `roll`, `std_id`, `std_name`, `fathers_name`, `mothers_name`, `gender`, `tc_no`, `tc_date`) VALUES ('".$fetchForall["class_name"]."','".$fetchForall["class_roll"]."','".$_GET["stdid"]."','".$fetchForall["student_name"]."','".$fetchForall["father_name"]."','".$fetchForall["mother_name"]."','".$fetchForall["gender"]."','".$_GET["stdid"]."','".date('d-m-Y')."')");
		    print "<script>alert('Save Successfully!!')";
  ...
  <tr>
    ...
    <td width="307" valign="bottom"><table width="100%" border="0" cellpadding="0" cellspacing="0">
     ...
     <!-- <tr>
      ...
      <tr>
        <td><span style="font-size:20px; font-weight:bold;">Date</span></td>
        <td><span class="style19">:</span></td>
        <td><span style="font-size:20px; font-weight:bold;"><?php print $_GET["date"]?></span></td>
      </tr>
      <tr>
        <td><span style="font-size:20px; font-weight:bold;">Sl. No.</span></td>
        <td><span class="style19">:</span></td>
        <td><span style="font-size:20px; font-weight:bold;"><?php print $_GET["stdid"]?></span></td>
      </tr>
    </table></td>
  </tr>
  <tr>
...
<input type="submit" value="Save"  style="height:30px; width:80px; background:green; "class="dont-print"  name="save"/>
