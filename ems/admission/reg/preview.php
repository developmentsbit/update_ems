...
error_reporting(0);
  ...
  $id=$_GET['id'];
   ...
   $sql ="SELECT `preview_reg_student_acadamic_information`.*,`preview_reg_student_address_info`.*,`preview_reg_student_guardian_information`.*,
`preview_reg_student_personal_info`.*, `preview_reg_student_previous_result`.*,`preview_reg_student_passward`.*,`add_class`.`class_name`,
`add_group`.`group_name` FROM `preview_reg_student_acadamic_information` INNER JOIN `preview_reg_student_address_info` ON 
`preview_reg_student_address_info`.`id`=`preview_reg_student_acadamic_information`.`id` INNER JOIN `preview_reg_student_guardian_information` 
ON `preview_reg_student_acadamic_information`.`id`=`preview_reg_student_guardian_information`.`id` INNER JOIN `preview_reg_student_personal_info`
 ON `preview_reg_student_personal_info`.`id`=`preview_reg_student_acadamic_information`.`id` INNER JOIN `preview_reg_student_previous_result` 
 ON `preview_reg_student_previous_result`.`id`=`preview_reg_student_acadamic_information`.`id` INNER JOIN `preview_reg_student_passward` 
 ON `preview_reg_student_passward`.`studentId`=`preview_reg_student_acadamic_information`.`id` INNER JOIN `add_class` 
 ON `add_class`.`id`=`preview_reg_student_acadamic_information`.`admission_disir_class` INNER JOIN `add_group` 
 ON `add_group`.`id`=`preview_reg_student_acadamic_information`.`admission_disir_group` 
WHERE `preview_reg_student_acadamic_information`.`id`='$id'";
