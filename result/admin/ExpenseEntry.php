...
if(isset($_POST['add']))
	{
		...
		{
			$query_ass="INSERT INTO `other_cost` (`id`,`date`,`title`,`description`,`amount`,`admin_id`,`Entry_Date`) VALUES ('$id','$d','$title','$description','$amount','$adminame','".date('Y-m-d')."')";
...
if(isset($_POST['Update']))
	{
		...
		if(!empty($id) && !empty($date) && !empty($title) && !empty($amount) && !empty($adminame))
		{
			$query_ass="REPLACE INTO `other_cost` (`id`,`date`,`title`,`description`,`amount`,`admin_id`,`Entry_Date`) VALUES ('$id','$d','$title','$description','$amount','$adminame','".date('Y-m-d')."')";
...
?id=$a[0]' class='btn btn-success'>Report</a>";
			$table.="</td>";
