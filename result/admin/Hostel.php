<?php
  ...
  $select="SELECT * FROM `hostelinformation`";
   ...
   if($checked_query)
    ...
    if(isset($_POST['add']))
    {
      ...
      $about_school=$db->escape($_POST['Contact_Textarea']);
    	...
    	{
    	$insert="INSERT INTO `hostelinformation` VALUES('1','$title','$about_school')";
  ...
  if(isset($_POST['Update']))
    {
      ...
      {
      	$insert="REPLACE INTO `hostelinformation` VALUES('1','$title','$about_school')";
...
if(isset($_POST['Delete'])){
	$Delete="DELETE FROM `hostelinformation` WHERE `id`='1'";
	$db->delete_query($Delete);
  print "<script>location='Hostel.php'</script>";
...
<body>
  ...
  <div class="panel-heading" style="font-size: 16px; color: #16a085; text-transform: uppercase;"><b>Hostel Information</b></div>
  <div class="panel-body">
   <form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
    ...
    <label >Title  :</label>
  <input type="text"  name="title" <?php if($checked_query){?> value="<?php echo $fetch_zero[1];?>"<?php } else {?> value="" <?php } ?> class="form-control" placeholder="Title" style="height: 40px; border-radius: 0px;" />
  </div>  
