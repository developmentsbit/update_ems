<?php
 error_reporting(1);
    ...
    if(isset($_POST['add']))
	{
		...
		$amount = $db->escape(isset($_POST['amount'])?$_POST['amount']:"0");
		$durationto = $db->escape($_POST['duration']);
		
		if($Title_type!="Select Type" && !empty($title))
		{
			$query="INSERT INTO `income_expense_title` (`title`,`details`,`Duration`,`amount`,`type`,`index`) VALUES ('$title','$detisls','$durationto','$amount','$Title_type','$index')";
...
if(isset($_POST['Update']))
	{
		...
		if($Title_type!="Select Type" && !empty($title))
		{
			$query="REPLACE INTO `income_expense_title` (`id`,`title`,`details`,`Duration`,`amount`,`type`,`index`) VALUES ('$id','$title','$detisls','$durationto','$amount','$Title_type','$index')";
			echo $query;
...
//view data...............................	
	if(isset($_POST['View']))
	{
		...
		$table.="<table class='table table-responsive table-hover table-bordered'>"."<tr class='success'>"."<td >SL </td>"."<td >Type</td>"."<td >Title</td>" ."<td >Details </td>"."<td >Durotion </td>"."<td >Amount </td>"."<td>Edit Or Delete</td>"."</tr>";
		if($result)
		{
		...
		while($a=$result->fetch_array())
		{
			   $table.="<tr>";
				...
				$table.="<td>".$a['duration']."</td>";
				$table.="<td>".$a['amount']."</td>";
...
function check_availability()
{
  ...
  <div class="panel-heading" style="font-size: 16px; color: #16a085; text-transform: uppercase;"><b>Income Expense Title</b></div>
  <div class="panel-body">
   ...
   <input type="text" placeholder="Amount" style="height: 40px; border-radius: 0px;" autocomplete="off" class="form-control" name="amount" value="<?php echo $fetch[4];?>" />
    ...
    <label >Duration :</label>
    	<select class="form-control" name="duration"  style="height: 40px; border-radius: 0px;"> 
