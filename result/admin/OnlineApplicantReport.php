...
<style>
  @media print{
	...
	<table width="750" align="center" style="border: 1px #000 solid; height: 80px; ">
		<tr>	
				...
				<td style="border: 0px;">
						...
						<div style="float: left; clear: right; text-align: center; width: 70% ">   
							   ...
							   <li style="list-style: none; ">
							    ...
							    <li style=" list-style: none; font-size:14px;font-family:Arial, Helvetica, sans-serif; font-size: 18px; font-weight: bold;">Applicant List</li>
							 </ul> 
...
ON `reg_student_previous_result`.`id`=`reg_student_passward`.`studentId` GROUP BY `reg_student_personal_info`.`email`";
				...
				?>
							...
							<tr>
								<td align="center" colspan="10" style="border-left: 1px #000 solid"> <span class="text-success" style="font-size:18px;"><?php echo $fetchSEssion["email"];?></span>
								</td>
							</tr>
							<?php 
...
ON `reg_student_personal_info`.`id`=`reg_student_acadamic_information`.`id`
 ...
 WHERE `reg_student_personal_info`.`email`='$fetchSEssion[email]' GROUP BY `reg_student_acadamic_information`.`admission_disir_class` ORDER BY `reg_student_acadamic_information`.`admission_disir_class` ASC";
							...
							?>
							<tr>
								<td align="center" colspan="9" style="border-left: 1px #000 solid; background: #f4f4f4;height: 30px;"><span class="text-info" style="font-size:18px; font-weight: bold; "><?php echo $fetchForClass["class_name"];?> </span>
								</td>
							</tr>
						
							<?php 
									$sqlfordetails  = "SELECT `reg_student_personal_info`.*,`reg_student_acadamic_information`.*,`reg_student_passward`.*,`add_group`.`group_name`,`reg_student_guardian_information`.* FROM `reg_student_personal_info`
 INNER JOIN `reg_student_acadamic_information` ON `reg_student_acadamic_information`.`id`=`reg_student_personal_info`.`id` INNER JOIN `reg_student_guardian_information` ON `reg_student_guardian_information`.`id`=`reg_student_personal_info`.`id` INNER JOIN `reg_student_passward` ON `reg_student_passward`.`studentId`=`reg_student_personal_info`.`id` INNER JOIN `add_group` ON
 `add_group`.`id`=`reg_student_acadamic_information`.`admission_disir_group`
 WHERE  `reg_student_personal_info`.`email`='".$fetchSEssion['email']."' AND
 `reg_student_acadamic_information`.`admission_disir_class`='".$fetchForClass["admission_disir_class"]."' ORDER BY `reg_student_personal_info`.`id` ASC";
							...
							<tr>
							   ...
							   <td align="center"><?php echo $fetchfordetails["father_name"];?></td>
								...
								<?php echo $fetchfordetails["guardian_contact"];?> </td>
							</tr>
							<?php } } } } } ?>

								<tr>
								<td align="center" colspan="10" style="border-right: 0px;">
