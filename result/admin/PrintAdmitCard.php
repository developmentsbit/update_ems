...
error_reporting(1);
	...
	$db = new database();
		...
		if(isset($_POST["ResultGenerate"])){
			...
			$stdRoll=$_POST["stdId"];
			if(isset($classID) and $classID != "Select One" and isset($examtype) and $examtype != "Select Exam Name"  and isset($stdRoll) and !empty($stdRoll)){
					print "<script>location='admit/admit.php?clID=$classID[0]&exId=$examtype[0]&session=$Session&gpna=$groupname[0]&stdRoll=$stdRoll'</script>";
			}else{
  ...
  <body>
  	<form name="teacherinfo" action="" method="post"  enctype="multipart/form-data" class="form-horizontal marksEditEntry" >
            <div class="col-md-8 col-md-offset-2" id="AddMarksStep1"><br/> <br/>
                <span class="glyphicon glyphicon-arrow-right text-justify text-uppercase text-warning" style="font-weight: bold; padding-top: 15px; font-size: 18px;"><strong style="padding-left:5px">Print Admit Card </strong></span><br/>
                ...
                <table class="table table-responsive table-hover table-bordered" style="margin-top:10px;">
				    ...
				    <tr>
                        ...
                        <td><div class="col-md-8"><select class="form-control" name="examtype" id="examtype">
                        </select>
                        </div>
                        </td>
                    </tr>
