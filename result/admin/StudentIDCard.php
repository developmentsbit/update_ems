	...
	$db = new database();
 		...
 		if(isset($_POST["ResultGenerate"])){
			...
			if(isset($classID) and $classID != "Select One" and isset($examtype) and $examtype != "Select Exam Name" and isset($Session) and $Session != "Select Session" ){
					print "<script>location='ajaxForStudentIDcard.php?clID=$classID[0]&session=$Session&gpna=$groupname[0]&stdRollfrom=$stdRollfrom&stdRollto=$stdRollto'</script>";
			}else{
  ...
  <body>
  	<form name="teacherinfo" action="" method="post"  enctype="multipart/form-data" class="form-horizontal marksEditEntry" >
            <div class="col-md-8 col-md-offset-2" id="AddMarksStep1"><br/> <br/>
                <span class="glyphicon glyphicon-arrow-right text-justify text-uppercase text-warning" style="font-weight: bold; padding-top: 15px; font-size: 18px;"><strong style="padding-left:5px">Generate Student ID Card </strong></span><br/>
                ...
                <tr>
                    <td ><strong><span class="text-success" style="font-size: 15px;">Session</span></strong></td>
                        <td><div class="col-md-8"><select class="form-control" id="session" name="session">
...
<input type="text" name="to" id="to"  style="width:120px; background:#F7071D; border:none; font-weight:bold; color:#fff; height:25px;  text-align:center;"/>
				...
				<tr>
					<Td colspan="2" align="right">
								...
								<strong class="text-capitalize text-danger">Limit Must be less than 10 or Equal....!!</strong>
								
					</Td>
				</tr>
				
                <tr><td colspan="2" align="center"><input type="submit" name="ResultGenerate" value="Show ID Card" class="btn btn-danger btn-md" style="width: 180px" formtarget="_blank"></input>
