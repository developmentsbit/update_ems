...
<?php }  }
			...
			?>
					<div class="col-md-10 col-md-offset-1">
							<table class="table table-responsive table-hover table-bordered "> 
									<tr>
											<td colspan="4" align="center"><strong class="text-success" style="font-size:15px; ">Testimonial </strong></td>
									</tr>
 ...
 if(isset($_GET["staticid"])){
			$sql="SELECT * FROM `static_testomonial` GROUP BY `Title` ASC";
			$resultSql=$db->select_query($sql);
				...
				<table width="62%" class="table table-responsive table-hover table-bordered">
							<tr>
								<td align="right" colspan="10">
									...
									<input type="button" onclick="return creatAdmin()" class="btn btn-success btn-sm"  value="Create Testimonial (Honours /Degree)" />
										<input type="button" class="btn btn-danger btn-sm" style="width:120px;" onclick="return deleteFunction()" value="Delete" />
											<input type="submit" class="btn btn-info btn-sm" style="width:120px;" value="Represh" />								</td>
							</tr>
							<?php 
							...
							<tr>
								...
								<?php 
										$sqlsession="SELECT * FROM `static_testomonial` WHERE `Title`='".$fetchSql["Title"]."' GROUP BY `Session` ORDER BY `Session` DESC";
										$resultSession=$db->select_query($sqlsession);
											if($resultSession){
													while($fetchResult=$resultSession->fetch_array()){
								?>
									<tr>
										...
										<?php 
											$sqlForAll="SELECT * FROM `static_testomonial` WHERE `Title`='".$fetchResult["Title"]."' AND `Session`='".$fetchResult["Session"]."'";
											$resulForAll=$db->select_query($sqlForAll);
											if($resulForAll){
												while($fetchforAll=$resulForAll->fetch_array()){
										?>
										<tr align="center">
											...
											<td width="17%">
												...
												<label> <a href="StaticviewTestimonial2.php?date=<?php echo date('d/m/Y')?>&stdid=<?php echo $fetchforAll["boardResultID"];?>" class="btn btn-success btn-sm" target="_blank">Show Testimonial</a></label>

												


												<label> <a href="editTestomonial.php?id=<?php echo $fetchforAll["boardResultID"];?>" class="btn btn-info btn-sm"  target="adminbody">Edit</a> </label>
...
<?php if(isset($_POST["statNotid"])){
          ...
          $(document).ready(function () {
                ...
                $('#example2').datepicker({
					...
					<div class="col-md-10 col-md-offset-1">
							<table class="table table-responsive table-hover table-bordered "> 
									<tr>
											<td colspan="4" align="center"><strong class="text-success" style="font-size:15px; ">Testimonial Info</strong></td>
									</tr>
										<tr>
											<td>Type</td>
											<td>:</td>
										...
										<select id="Type" name="Type" class="form-control">																
											...
											<td>
											<div class="col-md-8">
												<input type="text" placeholder="dd-mm-yy" id="example2" name="dataTesto" class="form-control" autocomplete="off"/>
											  </div>
											</td>
									</tr>
							...
							<!-- 		<tr>
									...
									<tr>
										<td>Student Name</td>
											...
											<td>
											<div class="col-md-8">
													<input type="text" class="form-control" name="studentName" id="studentName" placeholder="student  Name" autocomplete="off" />
											  </div>
											</td>
									</tr>
									...
									<tr>
										<td>Father Name</td>
											...
											<td>
											<div class="col-md-8">
													<input type="text" class="form-control" name="fatherName" id="fatherName" placeholder="Father Name" autocomplete="off"/>
											  </div>
											</td>
									</tr>
										...
										<td>Mother Name</td>
											...
											<td>
											<div class="col-md-8">
													<input type="text" class="form-control" name="MotherName" id="MotherName" placeholder="Mother Name" autocomplete="off" />
											  </div>
											</td>
									</tr>
									<tr>
									...
									<tr>
										<td>Roll No</td>
											...
											<td>
											<div class="col-md-8">
													<input type="text" class="form-control" name="RollNO" id="RollNO" placeholder="Roll NO.." autocomplete="off"/>
											  </div>
											</td>
									</tr>
									<tr>
										<td>Reg No</td>
											...
											<td>
											<div class="col-md-8">
													<input type="text" class="form-control" name="RegNo" id="RegNo" placeholder="Reg No.." autocomplete="off"/>
											  </div>
											</td>
									</tr>
								<tr>
										<td>GPA</td>
											...
											<td>
											<div class="col-md-8">
													<input type="text" class="form-control" name="GPA" id="GPA" placeholder="GPA.." autocomplete="off"/>
											  </div>
											</td>
									</tr>
		...
		//rint "dd";
			$bdid=$db->withoutPrefix('static_testomonial','boardResultID','1985','10');
					if(!empty($_POST["type"]) && isset($_POST["type"]) && !empty($_POST["type"]) && isset($_POST["type"])  && !empty($_POST["RollNO"]) && isset($_POST["RollNO"]) && !empty($_POST["session"]) && isset($_POST["session"]) && !empty($_POST["Year"]) && isset($_POST["Year"])  && !empty($_POST["gpa"]) && isset($_POST["gpa"])
					...
					&& !empty($_POST["MotherName"]) && isset($_POST["MotherName"])){
							 ...
							 $inssql="INSERT INTO `static_testomonial` (`boardResultID`,`type`,`Title`,`GroupName`,`Session`,`year`,`RollNo`,`RegNo`,`GPA`,`studentName`,`fatherName`,`motherName`,`gender`) VALUES ('$bdid','".$_POST["type"]."','".$_POST["type"]."','".$_POST["group"]."','".$_POST["session"]."','".$_POST["Year"]."','".$_POST["RollNO"]."','".$_POST["regNo"]."','".$_POST["gpa"]."','".$_POST['studentName']."','".$_POST['fatherName']."','".$_POST['MotherName']."','".$_POST["gender"]."')";
								$result=$db->insert_query($inssql);
										
					 $inserSql="INSERT INTO `distributedtestomoniallist` (`date`,`studentId`) VALUES ('".$_POST["example2"]."','$bdid')";
			...
			if(isset($_POST["sdchekid"]))
		{
			...
			for($x = 0; $x < $countSelect; $x++){
				  $delet_query="DELETE FROM `static_testomonial` WHERE `boardResultID`='".$_POST["chekvalue"][$x]."'";
				 $db->delete_query($delet_query);
				 $db->delete_query("DELETE FROM `distributedtestomoniallist`WHERE `studentId`='".$_POST["chekvalue"][$x]."'");
