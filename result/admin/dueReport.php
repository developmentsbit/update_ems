...
<tr>
	<td colspan="9"  style="border-left: 1px #000 solid; border-top: 1px #000 solid;border-right: 1px #000 solid; font-weight: bold; height: 40px; width: 30px; "> 
...
<h3 style="float: left; clear: right;">Month :  
			...
			WHERE `add_fee`.`fk_month_id`='0$i' AND `student_account_info`.`studentID`='$fetchsql[studentID]' AND `student_account_info`.`year`='".$_POST['year']."' AND 
...
WHERE `add_discount`.`student_id`='$fetchsql[studentID]' AND `add_discount`.`class_id`='".$_POST['className']."' AND `add_discount`.`year`='".$_POST['year']."' AND `add_fee`.`fk_month_id`='0$i'";
...
WHERE `student_paid_table`.`student_id`='$fetchsql[studentID]' AND `student_paid_table`.`year`='".$_POST["year"]."' AND `student_paid_table`.`month`='0$i'";
