<?php
    ...
    global $chek_query;
	 $prefix=date("y"."m"."d");
	$fetch_query[0]=$db->withoutPrefix('pay','pay_id',"8".$prefix,'10');
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","");         
                ...
                @$inserquery="INSERT INTO `pay` (`pay_id`,`date_time`,`title`,`extension`,`description`) VALUES('".$fetch_query[0]."','$date','$title','$file_ext','$detaile')";
...
if(isset($_POST["view"]))
{
    $query="select * from pay ORDER BY `Date_time` DESC";
        ...
        $table.="<tr>"."<td align='left'>"."<a href='pay.php' class='btn btn-primary'>"."<span class='link text-center'>Back<span>"."</a>"."</td>".
        "<td colspan='6' align='center'>"."<span class='text-success' style='font-size:18px; font-weight:blod;'>View Fee Information </span>"."</td>"."</tr>";
        if($result){
            $table.="<tr>"."<td>Date</td>"."<td>Title</td>"."<td>Fee Details Or File</td>"."<td>Edit Or Delete</td>"."</tr>";
...
if(isset($_GET["dlt"]))
{
    $Delete="DELETE FROM `pay` WHERE `pay_id`='".$_GET["dlt"]."'";
    $db->delete_query($Delete);
    print "<script>location='pay.php'</script>";
$fetch_query[0]=$db->withoutPrefix('pay','pay_id',"8".$prefix,'10');
...
if(isset($_GET["edit"]))
{
        $select_query="SELECT * FROM `pay` WHERE `pay_id`='".$_GET["edit"]."'";
    ...
    {   
        ...
        $expensions= array("jpeg","jpg","png","pdf","");         
                ...
                @$inserquery="REPLACE INTO `pay` (`pay_id`,`date_time`,`title`,`extension`,`description`) VALUES('".$_POST["id"]."','$date','$title','$file_ext','$detaile')";
...
<script type="text/javascript">
 ...
 function checkforText()
    ...
    <?php if(isset($_POST["view"]))
    {
        if(!$result)
        {
            echo "<span class='text-danger' style='font-size:22px;'>"."<strong>"."No Pay  Found"."<a href='pay.php'>"."Go Back"."</a>"."<strong>"."</span>";
        }
        else
        {
        echo $table;
...
Details &nbsp;</td>
        <td>:</td>
         <td>
            <div class="col-sm-12 col-md-12">
            <textarea  name="descreption" class="form-control textarea"  id="redactor" disabled="disabled"   ><?php if($chek_query){echo $fetch_query[4]; } else { echo ""; }?> </textarea>
            </div>
            </td>
