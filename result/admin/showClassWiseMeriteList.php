...
JOIN `add_class` ON `add_class`.`id`=`result`.`classId` 

JOIN `exam_type_info` ON `exam_type_info`.`exam_id`=`result`.`examId`  WHERE `result`.`classId`='$clId'  AND `result`.`examId`='$examId' AND `result`.`session`='$session'";	
	//print $sql_2;
            ...
            <div class="col-md-10 col-md-offset-1"  style="border:2px #CCCCCC solid; margin-top:10px;"><br/> <br/>
                ...
                <table cellpadding="0" cellspacing="0"  class="table table-responsive table-hover " style="margin-top:0px;">					
							...
							<tr style="border:1px #CCCCCC solid" class="warning">
									...
									<td align="center" colspan="2"><span class="text-justify text-info" style="font-size:14px;"><strong>  	Session 	: 	<?php if($resultSql){ echo $session;}else {echo "";}?> </strong></span></td>
									<td align="right" colspan="2"></td>
							</tr>
							<tr class="" style="border:1px #CCCCCC solid">
									<td colspan="8" align="center"><span class="text-justify text-danger" style="font-size:16px;"><strong>  Merit List </strong></span> </td>
							</tr>
...
INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`result`.`STD_ID` INNER JOIN `student_acadamic_information` ON `student_acadamic_information`.`id`=`result`.`STD_ID` WHERE `result`.`classId`='$clId'  AND `student_acadamic_information`.`session2`='$session' AND `result`.`GroupID`='$gpId' AND `result`.`examId`='$examId' AND `result`.`CGPA` !='0.00' ORDER BY `CGPA` DESC $limit";
							...
							*/
								...
								$select="SELECT `CGPA` FROM `result`  WHERE `result`.`classId`='$clId' AND `result`.`session`='$session'  AND `result`.`examId`='$examId' AND `result`.`CGPA` !='0.00' GROUP BY `CGPA` ORDER BY `CGPA` DESC   ";
						...
						if($resultquery)
						{
							...
							$sql1= "SELECT `witoutOptional` FROM `result`  WHERE `result`.`classId`='$clId'  AND `result`.`session`='$session'  AND `result`.`examId`='$examId' AND `result`.`CGPA`=$fetch_gpa[0] GROUP BY `witoutOptional` ORDER BY `witoutOptional` DESC  ";
							...
							if($result1){
								...
								while($fetchWithoutOptionalGPA=$result1->fetch_array()){


									$selectResultOnRoll="SELECT `result`.*,`student_personal_info`.`student_name`,`father_name`,`mother_name` FROM `result`
INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`result`.`STD_ID`  WHERE `result`.`classId`='$clId'  AND `result`.`session`='$session'  AND `result`.`examId`='$examId' AND `result`.`CGPA`=$fetch_gpa[0] AND `witoutOptional`=$fetchWithoutOptionalGPA[0]  ORDER BY `std_roll` ASC  ";

							$resultWithoutOptional=$db->select_query($selectResultOnRoll);
							if($resultWithoutOptional){
						
								while($fetchAll=$resultWithoutOptional->fetch_array())
							    {
								$sl++;
						...
						<?php
							...
							<tr class="active" style="border:1px #CCCCCC solid">
									<td colspan="8" align="center" class="dontPritntd"><input type="button" name="print" id="print" value="Print"  onClick="window.print()" class="btn btn-danger btn-sm noneBtnForprin"/> </td>
							</tr>
				</table>
