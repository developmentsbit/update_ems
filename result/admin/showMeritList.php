...
INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`result`.`STD_ID` INNER JOIN `student_acadamic_information` ON `student_acadamic_information`.`id`=`result`.`STD_ID` WHERE `result`.`classId`='$clId'  AND `student_acadamic_information`.`session2`='$session' AND `result`.`GroupID`='$gpId' AND `result`.`examId`='$examId' AND `result`.`CGPA` !='0.00' ORDER BY `CGPA` DESC $limit";
							...
							*/
								$sl =0;
							$select="SELECT `CGPA` FROM `result`  WHERE `result`.`classId`='$clId' AND `result`.`session`='$session' AND `result`.`GroupID`='$gpId' AND `result`.`examId`='$examId' AND `result`.`CGPA` !='0.00' GROUP BY `CGPA` ORDER BY `CGPA` DESC   ";
							//print $select;
						...
						if($resultquery)
						{
							...
							$sql1= "SELECT `witoutOptional` FROM `result`  WHERE `result`.`classId`='$clId'  AND `result`.`session`='$session' AND `result`.`GroupID`='$gpId' AND `result`.`examId`='$examId' AND `result`.`CGPA`=$fetch_gpa[0] GROUP BY `witoutOptional` ORDER BY `witoutOptional` DESC  ";
							$result1=$db->select_query($sql1);
...
INNER JOIN `student_personal_info` ON `student_personal_info`.`id`=`result`.`STD_ID`  WHERE `result`.`classId`='$clId'  AND `result`.`session`='$session' AND `result`.`GroupID`='$gpId' AND `result`.`examId`='$examId' AND `result`.`CGPA`=$fetch_gpa[0] AND `witoutOptional`=$fetchWithoutOptionalGPA[0]  ORDER BY `std_roll` ASC  ";
